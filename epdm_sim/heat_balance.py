...
from .dimensioned import ensure_power_kW
from .reaction_templates import heat_balance_deltaH_from_template
...
class HeatBalanceConfig(BaseModel):
    ...
    deltaH_polymerization: dict[str, float] = Field(default_factory=heat_balance_deltaH_from_template)
...
def calculate_reaction_heat(
    ...
    heat_sum = 0.0
    for monomer, consumed_mol_h in mol_consumed_h.items():
        heat_sum += positive(consumed_mol_h) * float(delta_h.get(monomer, 0.0))
...
def calculate_heat_balance(
    ...
    if not isinstance(preheat_kJ_h, (int, float)):
        preheat_kJ_h = ensure_power_kW(preheat_kJ_h, default_unit="kJ/h") * 3600.0
    if not isinstance(devol_kJ_h, (int, float)):
        devol_kJ_h = ensure_power_kW(devol_kJ_h, default_unit="kJ/h") * 3600.0
    if not isinstance(sensible_heat_kJ_h, (int, float)):
        sensible_heat_kJ_h = ensure_power_kW(sensible_heat_kJ_h, default_unit="kJ/h") * 3600.0
