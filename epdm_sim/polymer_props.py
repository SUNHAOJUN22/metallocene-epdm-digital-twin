...
def predict_polymer_properties(template_id: str, composition: dict[str, float], Mw: float, PDI: float, process_conditions: dict[str, float] | None = None, params: dict[str, Any] | None = None):
    """Dispatch polymer property predictions through the V4.5 template layer."""
    from .property_models import predict_polymer_properties as _dispatch

    return _dispatch(template_id, composition, Mw, PDI, process_conditions, params)
...
def grade_target_value(grade: dict[str, Any], metric: str) -> float:
    """Return a single target value for a grade metric."""
    target_key = {
        "C2": "target_C2_wt",
        "ENB": "target_ENB_wt",
        "ML": "target_Mooney",
    ...
    if target_key in grade:
        return float(grade[target_key])
    if metric == "C2" and "C2_min" in grade:
        return 0.5 * (float(grade["C2_min"]) + float(grade["C2_max"]))
    if metric == "ENB" and "ENB_min" in grade:
        return 0.5 * (float(grade["ENB_min"]) + float(grade["ENB_max"]))
    if metric == "ML" and "ML_min" in grade:
        return 0.5 * (float(grade["ML_min"]) + float(grade["ML_max"]))
    return 0.0


def grade_bounds(grade: dict[str, Any], metric: str, default_span: float) -> tuple[float, float]:
    """Return min/max grade bounds, deriving them from target when needed."""
    if metric == "C2" and "C2_min" in grade:
        return float(grade["C2_min"]), float(grade["C2_max"])
    if metric == "ENB" and "ENB_min" in grade:
        return float(grade["ENB_min"]), float(grade["ENB_max"])
    if metric == "ML" and "ML_min" in grade:
        return float(grade["ML_min"]), float(grade["ML_max"])
    target = grade_target_value(grade, metric)
    return target - default_span / 2.0, target + default_span / 2.0
...
def grade_match(product: dict[str, float], grade_id: str) -> dict[str, Any]:
    ...
    product_values = {
        "C2": float(product.get("C2_wt", product.get("ethylene_wt", 0.0))),
        "ENB": float(product.get("ENB_wt", 0.0)),
        "ML": float(product.get("Mooney", product.get("mooney", 0.0))),
        "Mw": float(product.get("Mw", 0.0)),
        "PDI": float(product.get("PDI", 0.0)),
    }
    targets = {
        "C2": grade_target_value(grade, "C2"),
        "ENB": grade_target_value(grade, "ENB"),
        "ML": grade_target_value(grade, "ML"),
        "Mw": grade_target_value(grade, "Mw"),
        "PDI": grade_target_value(grade, "PDI"),
    }
    weights = {"C2": 1.0, "ENB": 1.4, "ML": 1.0 / 10.0, "Mw": 1.0 / 100000.0, "PDI": 0.8}
    deviations: dict[str, float] = {}
    weighted_error = 0.0
    for metric in ["C2", "ENB", "ML"]:
        deviations[metric] = product_values[metric] - targets[metric]
        weighted_error += weights[metric] * abs(deviations[metric])
    if targets["Mw"] > 0.0:
        deviations["Mw"] = product_values["Mw"] - targets["Mw"]
        weighted_error += weights["Mw"] * abs(deviations["Mw"])
    if targets["PDI"] > 0.0:
        deviations["PDI"] = product_values["PDI"] - targets["PDI"]
        weighted_error += weights["PDI"] * abs(deviations["PDI"])

    penalties = {}
    for metric, default_span in [("C2", 6.0), ("ENB", 1.5), ("ML", 20.0)]:
        low, high = grade_bounds(grade, metric, default_span)
        value = product_values[metric]
        ...
        penalties[metric] = float(penalty)

    score = clamp(100.0 / (1.0 + weighted_error / 10.0), 0.0, 100.0)
    return {
        "grade_id": grade_id,
        "grade": grade,
        "score": score,
        "match_error": weighted_error,
