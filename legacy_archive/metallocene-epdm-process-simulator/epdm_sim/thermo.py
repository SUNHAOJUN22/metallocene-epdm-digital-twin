...
class ThermoEngine:
    ...
    def __init__(self, mode: str = "Simple Wilson K", components: dict[str, Component] | None = None):
        ...
        self.mode = "thermo-backed" if mode.startswith("thermo") and thermo_package_available() else "Simple Wilson K"
    ...
    def k_values(self, names: list[str], temperature_K: float, pressure_Pa: float) -> dict[str, float]:
        """Return K values for selected component names.

        In thermo-backed mode, this first attempts the optional thermo.Chemical
        Psat interface and falls back component-by-component to Wilson K.
        """
    ...
    def flash(self, z_moles: dict[str, float], temperature_K: float, pressure_Pa: float) -> FlashSplit:
        ...
        z = z / np.sum(z)
        k_map = self.k_values(names, temperature_K, pressure_Pa)
