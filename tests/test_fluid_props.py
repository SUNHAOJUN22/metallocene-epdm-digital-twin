...
from epdm_sim.fluid_props import apparent_viscosity, calculate_pipe_hydraulics
...
def test_non_newtonian_apparent_viscosity_shear_thins():
    low_shear = apparent_viscosity(0.5, 0.1, "power-law")
    high_shear = apparent_viscosity(0.5, 100.0, "power-law")
    ...
    hydraulics = calculate_pipe_hydraulics(
        ...
        pipe_length_m=10.0,
        pipe_diameter_m=0.02,
        rheology_model="power-law",
    )
    assert hydraulics.apparent_viscosity_Pa_s > 0.0
    assert hydraulics.shear_rate_s > 0.0
